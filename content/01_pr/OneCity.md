---
title: OneCity
draft: false
tags:
  - gis
  - design
  - policy
date: 2019 — 2025
Location: Johannesburg, South Africa
My role: Project Lead, Urban Strategist
Team: NOVAYAlabs, SARChi (University of the Witwatersrand)
link:
  - https://onecity.dev/
cssclasses:
  - project
hide: true
---
## Spatial decision-support platform in Johannesburg

OneCity is a web-based GIS platform for analysing land plots in relation to official spatial strategies. It brings together planning, regulatory, and spatial datasets in a single interactive map interface to support development and investment decisions. 

At NOVAYA Labs, I worked as a Project Manager on the design and coordination of this platform. As part of a small team working with [SARChI at the University of the Witwatersrand](https://www.wits.ac.za/sacp/about-us/), I contributed to early concept development, prototyping and wireframing, user research, and the development of product narratives and use-case scenarios. 

The project started in 2019 in Johannesburg. We explored how municipal and other urban data across South Africa are shared and used by the private sector. As a response to urban sprawl, fragmented data sources, and administrative silos, we built a WebGIS platform that brings together data from more than 50 sources, making up-to-date information on city development available in one place. OneCity helps users identify properties that are not affected by major planning constraints, such as flood risk. It also provides detailed site-level information, including whether environmental assessments or other studies are required before construction.


> [!carousel]
> ![[03.jpg]]
> ![[02.jpg]]

Fig. 01 OneCity platform combining planning regulations, spatial indicators, and market data for comparative site analysis.

---
## OneCity Insights

In april 2023, we released a major update [OneCity Insights](https://onecity.dev/insights), an analytical layer built on top of the OneCity platform. Insights was built to make it easier to see which land is both legally usable and practically viable. We integrated Lens search tool, a feature that highlights development opportunities within a 600-metre radius of a selected area.

> We identified 181 753 opportunities in Johannesburg to develop or extend single-family detached houses that fit the present property market climate. 2 452, which is 1.5% of these opportunities, have high development potential and are close to amenities and public transport. 

![[Pasted image 20260210012958.png]]
Fig. 02 OneCity Insights interface of interactive map, spatial, and market indicators for site-level development assessment (Johannesburg, 2023).

The first release focused on Johannesburg and assessed large numbers of land parcels for detached housing potential. Parcels were evaluated against planning regulations, land-use rights, parcel size and geometry, access to public transport, proximity to everyday amenities, and surrounding property values. Instead of browsing datasets one by one, users could compare sites through a single, structured view. The analysis surfaced more than 180 000 potential development sites, but also made clear how narrow the pool of genuinely viable land is. Only a small fraction of parcels met both regulatory and accessibility thresholds, making the trade-offs between planning rules, location, and market conditions explicit rather than assumed.

![[1_99RGuNSsXkg7QQsO96uJWg.gif]]
Fig. 02 OneCity Stages: we decide on Typology, filter out sites that do not fit our purpose or with high risks, then sort development opportunities from the best to the worst, and, finally, add property market data for each site.

We have evaluated more than 200 single-family development projects in Johannesburg and Pretoria to identify the most sought-after housing typology. The results provide a wide range of development opportunities varying in parcel dimensions, floor size, and the number of bedrooms. See identified typologies in Fig.03.

![[1_S2Q1BZr3Zax0nLAcRmYclA.webp]]
Fig.03: OneCity typologies

Here is the story on [Medium](https://one-city.medium.com/why-we-are-building-onecity-c0b0e573b51c) of how we developed OneCity Insights, step by step.

We also publish occasional updates on policy changes in South Africa, municipal portals, and the Johannesburg Spatial Development Framework. The newsletter focuses on practical issues around zoning and development landscape. You can find our blog [here](https://onecity.dev/)